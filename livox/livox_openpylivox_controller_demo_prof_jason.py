...
'''
Prof Jason's modification of 
https://github.com/ryan-brazeal-ufl/OpenPyLivox/blob/master/livox_controller_demo.py

This saves two .bin files (lidar and IMU data) and then converts to two .csv files.

It works with the Avia, but only up to 2 returns. We'll look into fixing that later.

To install the library (preferaby in a virtual environment), run:

# pip install git+https://github.com/ryan-brazeal-ufl/openpylivox.git

note that this got forked but not updated to become https://github.com/Livox-SDK/openpylivox
...
def singleSensorDemo():
    ...
    sensor = opl.openpylivox(showMessages=True)  # optional True/False, is used to have sensor messages printed to screen (default = False)
